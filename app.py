...
def fetch_financial_data(ticker_name):
    ...
    prompt = f"""
    株式アナリストとして、Google検索を駆使して{ticker_name}の最新（2024年〜2025年）の財務データを調査してください。
    
    以下の項目を必ず調べ、JSON形式のみで出力してください。説明文は一切不要です。
    - company_name: 会社名
    - current_price: 現在株価(円)
    - shares_outstanding: 発行済株式数(百万株)
    - net_debt: ネット有利子負債(百万円)
    - sales: 直近売上高(百万円)
    ...
    if not ticker:
        ...
        with st.spinner("Geminiが最新データを検索・分析中..."):
            try:
                ...
                c1.metric("算出された理論株価", f"¥{theoretical_price:,.0f}")
                c2.metric("現在の実際の株価", f"¥{current_price:,.0f}")
                upside = (theoretical_price / current_price - 1) * 100
                c3.metric("上昇余地", f"{upside:+.1f}%")
                
                with st.expander("Geminiの調査データ詳細を確認"):
                    st.json(data)
                st.bar_chart(pd.DataFrame({"予測FCF": future_fcf}, index=[f"{y}年目" for y in years]))
                
            except Exception as e:
                st.error("分析中にエラーが発生しました。時間を置いて再度お試しください。")
